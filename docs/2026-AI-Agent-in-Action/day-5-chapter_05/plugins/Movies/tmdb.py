...
async def test_tmdb():
    ...
    # 2. TMDb 서비스 플러그인 등록
    # 클래스의 인스턴스를 생성해서 등록합니다.
    tmdb_service = TMDbService()
    tmdb_plugin = kernel.add_plugin(tmdb_service, plugin_name="TMDbPlugin")

    print("--- TMDb 플러그인 테스트 시작 ---")

    # 테스트 1: 영화 장르 목록 가져오기
    print("\n[테스트 1: 영화 장르 목록]")
    genres = await kernel.invoke(tmdb_plugin["get_movie_genres"])
    print(f"결과: {genres}")

    # 테스트 2: 특정 장르(Action)의 현재 상영 영화 가져오기
    print("\n[테스트 2: 액션 영화 추천]")
    # 인자(Arguments)를 넘길 때는 KernelArguments를 사용합니다.
    args = KernelArguments(genre_name="Action")
    movies = await kernel.invoke(tmdb_plugin["get_top_movies_by_genre"], args)
    print(f"결과: {movies}")
...
if __name__ == "__main__":
    import asyncio
    # 만약 기존에 semantic_kernel이 설치되어 있다면 실행됩니다.
    try:
